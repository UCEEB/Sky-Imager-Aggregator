...
class Instrument():
    ...
    def __init__(self, slaveaddress,port,baudrate ,bytesize ,parity,stopbits,rtscts,dsrdtr, mode=MODE_RTU):
        ...
        if port not in _SERIALPORTS or not _SERIALPORTS[port]:
            self.serial = _SERIALPORTS[port] = serial.Serial(port=port, baudrate=BAUDRATE, parity=PARITY, bytesize=BYTESIZE, stopbits=STOPBITS,rtscts=RTSCTS,dsrdtr=DSRDTR, timeout=TIMEOUT)
        else:
